...
function DashboardPage() {
	...
	useEffect(() => {
		...
		const mapEventRow = (eventRow, index) => {
			const datetime = normalizeDateTime(eventRow?.event_date);
			const parsedDate = datetime ? new Date(datetime.replace(" ", "T")) : null;
			const derivedYear = eventRow?.event_year || (parsedDate && !Number.isNaN(parsedDate.getTime()) ? parsedDate.getFullYear() : null);
		...
		const loadEvents = async () => {
			...
			try {
				...
				const rawEvents = Array.isArray(eventData) ? eventData : Array.isArray(eventData?.data) ? eventData.data : [];
	...
	const filteredRows = selectedStatus === "All" ? sortedRows : sortedRows.filter((row) => row.status === selectedStatus);
		...
		<main className="dashboard-page">
			<section className="dashboard-card" aria-label="Dashboard">
				<div className="dashboard-header-row">
					<h1>View Events</h1>
					<button type="button" className="dashboard-add-button" aria-label="Add event" onClick={handleOpenCreateModal}>
						<span className="dashboard-add-button-icon">+</span>
					</button>
				</div>
				<div className="dashboard-table-toolbar">
					<button type="button" className="event-modal-action-btn" onClick={() => setIsInsightsModalOpen(true)}>
						Get Insights
					</button>
					<div style={{ flex: 1 }}></div>
					<div className="dashboard-filter-menu">
						<button type="button" className="dashboard-filter-button" onClick={() => setIsFilterOpen((previous) => !previous)} aria-haspopup="menu" aria-expanded={isFilterOpen}>
						...
						{isFilterOpen && (
							<div className="dashboard-filter-dropdown" role="menu" aria-label="Filter by status">
								{statusOptions.map((status) => (
									<button key={status} type="button" className={status === selectedStatus ? "active" : ""} role="menuitem" onClick={() => handleStatusSelect(status)}>
										{status}
									</button>
								))}
							</div>
				...
				<div className="event-grid" aria-label="Event records">
					<DashboardContent filteredRows={filteredRows} setSelectedEvent={setSelectedEvent} formatDateTime={formatDateTime} getDurationLeft={getDurationLeft} />
				</div>
